...
export default function Shop_weapons() {
    const [shopData, setShopData] = useState<any>(null)
    useNuiEvent<any>('setShopDataWeapons', (data) => {
        setShopData(data)
    })
                ...
                >
                    ...
                    <div className='w-full h-[18%] flex justify-between items-center bg-[#3b3b3a] drop-shadow p-[2%]'>
                        <div className='flex flex-row gap-3 items-center w-auto'>
                            <h1 className='bg-[#3b3b3a] 800:p-0 1280:p-1.5 p-0 800:px-3 1280:px-4 px-2 leading-none rounded text-white font-SFPro hover:bg-[#cdcdcd] hover:text-[#141313] cursor-pointer' onClick={() => {RemoveItemCart(data)}}>-</h1>
                            <h1 className='bg-[#3b3b3a] 800:p-0 1280:p-1.5 p-0 800:px-3 1280:px-4 px-2 leading-none rounded text-white font-Countach hover:bg-[#cdcdcd] hover:text-[#141313] cursor-pointer' onClick={() => {AddItemToCart(data)}}>+</h1>
                        </div>
    ...
    const checkOutRZ = () => {
        ...
        fetch('https://YBN/YBN.Shops:CheckOutRZ', options)
    }
    ...
    return (
        <div className="flex flex-col h-screen max-h-screen w-screen max-w-screen select-none p-[5%] pt-[3%] bg-black bg-opacity-50 bg-center bg-cover gap-[1%]">
            <div className='flex flex-row gap-[1%] items-end'>
                <div className='800:h-[20px] 800:w-[15px] 1024:h-[25px] 1024:w-[20px] 1280:h-[35px] 1280:w-[30px] 1600:h-[45px] 1600:w-[40px] 1920:h-[55px] 1920:w-[50px] 2560:h-[65px] 2560:w-[60px] bg-[#141618]/80 border border-dark-grey rounded 800:p-0.5 1024:p-1 1280:p-2 p-2 cursor-pointer'
                onClick={() => {
                    goBack()
                }}
                >
            ...
            <div className='w-full h-full flex flex-row gap-[1%] overflow-hidden'>
                ...
                <div className='gap-[3%] w-[42%] flex flex-col'>
                    {/* <div className='flex flex-col gap-[5%] h-[38%] bg-black/40'>
                        ...
                        <div className='flex flex-col gap-[2%] p-[5%] h-full w-full items-start justify-start'>
                            {
                                itemInfo !== null ? (
                                    <>
                                        <p className='text-left font-Countach tracking-wider uppercase leading-none rounded-sm 800:text-[10px] 1024:text-[12px] 1280:text-[15px] 1440:text-[16px] 1600:text-[18px] 1920:text-[20px] 2560:text-[22px] px-[0.5%] pb-1 bg-[#464946] text-[#9aa39a]'>{itemInfo.type}</p>
                                        <p className='text-left font-Countach tracking-wider uppercase leading 800:text-[10px] 1024:text-[15px] 1280:text-[20px] 1440:text-[25px] 1600:text-[30px] 1920:text-[40px] 2560:text-[50px] text-[#bbbcbc]'>{itemInfo.label}</p>
                                        <p className='text-left font-Countach tracking-wider font-light 800:text-[10px] 1024:text-[12px] 1280:text-[15px] 1440:text-[16px] 1600:text-[18px] 1920:text-[20px] 2560:text-[22px] text-[#9aa39a]'>{itemInfo.description}</p>
                                    </>
                                ) : (
                    ...
                    <div className='flex flex-col justify-around gap-[2%] h-full bg-black/40'>
                        ...
                        {
                            cartData !== null ? (
                                <div className='grid grid-cols-4 grid-flow-row auto-rows-max gap-[1%] px-[2%] pt-[1%] h-[80%] max-h-[80%] scroll-auto touch-auto overflow-auto'>
                                    {renderCartData()}
                                </div>
                            ) : (
...

                        {
                            getTotalPriceCart() !== 0 ? (
                                <div className='flex flex-row justify-end items-center p-[1%] px-[2%]'>
                                    {/* <h1 className='text-[#9aa39a] uppercase 800:text-[10px] 1024:text-[11px] 1280:text-[13px] 1600:text-[15px] 1920:text-[17px] 2560:text-[20px] font-Countach tracking-wide drop-shadow'>{languages["total"]}: <a className='text-yellow'>${priceToPriceWithCommase(getTotalPriceCart())}</a></h1> */}
                                    
                                    {/* <div className='flex flex-row gap-[3%] w-[70%] justify-end'>
                                        <h1 className='text-[#9aa39a] uppercase 800:text-[10px] 1024:text-[11px] 1280:text-[13px] 1600:text-[15px] 1920:text-[17px] 2560:text-[25px] font-Countach tracking-wide drop-shadow bg-dark-grey border border-dark-grey px-[0.5%] pb-1 rounded cursor-pointer'
                                        onClick={() => {SellCart()}}
